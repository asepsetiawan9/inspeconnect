...
@section('content')
@include('layouts.navbars.auth.topnav', ['title' => 'Edit Data Bantuan'])
<div class="row mt-4 mx-4">
    <div class="position-relative">
        <h5 class="text-white">Edit Data Bantuan</h5>
        <div class="card px-5 py-3">
            <form action="{{ route('assistance.update', $assistance->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-select" id="filter1" name="tahun">
                                ...
                                <option value="{{ $year }}" {{ $year == $assistance->tahun ? 'selected' : '' }}>
                                    {{ $year }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-select selectName" id="filter2" name="id_poverty">
                                ...
                                @foreach ($assistance->poverties as $poverty)
                                    <option value="{{ $poverty->id }}" {{ $poverty->id == $assistance->id_poverty ? 'selected' : '' }}>
                                        {{ $poverty->nama }} - ({{ $poverty->nik }})
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div id="formContainer">
                        @foreach ($assistance->assistDetails as $key => $detail)
                        <div class="formBantuan">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="nama_bantuan">Nama Bantuan</label>
                                        <input type="text" name="nama_bantuan[]" class="form-control"
                                            placeholder="Nama Bantuan" value="{{ $detail->nama_bantuan }}">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="pemberi_bantuan">Pemberi Bantuan</label>
                                        <input type="text" name="pemberi_bantuan[]" class="form-control"
                                            placeholder="Pemberi Bantuan" value="{{ $detail->pemberi_bantuan }}">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <textarea type="text" name="keterangan[]" class="form-control"
                                            placeholder="Keterangan">{{ $detail->keterangan }}</textarea>
                                    </div>
                                </div>
                                <div class="col-md-6">
...
<script>
    $(document).ready(function () {
        var formCount = {{ count($assistDetails ?? []) }}; // Jumlah form bantuan awal
